...
const BadgeGrid = ({ badges }) => {
  if (!badges || badges.length === 0) {
    return (
      <div style={{ background: 'white', borderRadius: 20, padding: '2rem', border: '1px solid #F0EDE8', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', textAlign: 'center', marginBottom: '2rem' }}>
        ...
        <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--color-heading)', margin: '0 0 0.5rem 0' }}>No Badges Yet</h3>
        <p style={{ fontSize: '0.85rem', color: 'var(--color-body)', margin: 0 }}>Keep contributing to earn badges and showcase your impact.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--color-heading)', margin: '0 0 1.25rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        {badges.map((badge, i) => {
          ...
          return (
            ...
            >
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--color-body)', margin: '0 0 0.5rem 0', lineHeight: 1.4 }}>{description}</p>
              {earnedAt && (
