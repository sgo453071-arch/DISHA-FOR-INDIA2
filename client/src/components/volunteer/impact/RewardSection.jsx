    ...
    >
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 120, background: 'linear-gradient(135deg, #FFFBEB, #FEF3C7)', borderRadius: 12, padding: '1rem', textAlign: 'center', border: '1px solid #FDE68A' }}>
          <div style={{ fontSize: '0.7rem', color: '#92400E', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total Points</div>
          <div style={{ fontSize: '1.8rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#B45309', lineHeight: 1.1 }}>{totalPoints}</div>
        </div>
        <div style={{ flex: 1, minWidth: 120, background: 'linear-gradient(135deg, #FFF7ED, #FFEDD5)', borderRadius: 12, padding: '1rem', textAlign: 'center', border: '1px solid #FED7AA' }}>
          <div style={{ fontSize: '0.7rem', color: '#9A3412', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total Coins</div>
          <div style={{ fontSize: '1.8rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#C2410C', lineHeight: 1.1 }}>{totalCoins}</div>
        </div>
      </div>
            ...
            {transactions.slice(0, 5).map((txn, i) => (
              <div
                ...
                style={{
                  ...
                  border: '1px solid #F0EDE8', fontSize: '0.8rem',
                }}
              >
                <div>
                  <div style={{ fontWeight: 600, color: 'var(--color-heading)', lineHeight: 1.3 }}>{txn.reason || txn.type || 'Reward'}</div>
                ...
                <span style={{ fontWeight: 700, color: (txn.points || txn.coins || 0) >= 0 ? '#059669' : '#DC2626' }}>
