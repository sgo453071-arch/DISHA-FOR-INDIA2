    ...
    >
      ...
      >
        ...
        <div style={{ padding: '1.5rem 2rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              {isProgram ? (
                <>
                  {recommendation.programCategory && (
                    <span style={{ fontSize: '0.8rem', padding: '0.4rem 0.875rem', background: 'rgba(37, 99, 235, 0.08)', color: 'var(--color-primary)', borderRadius: '999px', fontWeight: 600 }}>
                ...
                <>
                  ...
                  {recommendation.volunteerSkills?.length > 0 && (
                    <span style={{ fontSize: '0.8rem', padding: '0.4rem 0.875rem', background: 'rgba(16, 185, 129, 0.08)', color: '#059669', borderRadius: '999px', fontWeight: 600 }}>
          ...
          <div style={{ background: '#F9FAFB', borderRadius: 'var(--radius-md)', padding: '1rem', border: '1px solid #F0EDE8' }}>
            <h4 style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--color-heading)', margin: '0 0 0.5rem 0' }}>Matching Skills</h4>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {recommendation.matchingSkills?.length > 0 ? (
                recommendation.matchingSkills.map((skill, idx) => (
                  <span key={idx} style={{ fontSize: '0.78rem', padding: '0.3rem 0.65rem', background: 'white', color: 'var(--color-primary)', borderRadius: '999px', fontWeight: 600, border: '1px solid var(--color-border)' }}>
                    {skill}
                  </span>
                ))
              ) : (
                <p style={{ fontSize: '0.8rem', color: 'var(--color-body)', margin: 0 }}>No matching skills found</p>
              )}
            </div>
          </div>
          ...
          {recommendation.missingSkills?.length > 0 && (
            <div style={{ background: '#FEF2F2', borderRadius: 'var(--radius-md)', padding: '1rem', border: '1px solid #FECACA' }}>
              <h4 style={{ fontSize: '0.85rem', fontWeight: 700, color: '#991B1B', margin: '0 0 0.5rem 0' }}>Missing Skills</h4>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {recommendation.missingSkills.map((skill, idx) => (
                  <span key={idx} style={{ fontSize: '0.78rem', padding: '0.3rem 0.65rem', background: '#FEE2E2', color: '#991B1B', borderRadius: '999px', fontWeight: 500 }}>
                    {skill}
                  </span>
                ))}
              </div>
