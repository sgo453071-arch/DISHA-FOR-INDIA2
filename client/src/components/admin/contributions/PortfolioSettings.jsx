...
const PortfolioSettings = () => {
  ...
  const columns = [
    ...
    { key: 'value', label: 'Value', render: (val) => <span style={{ fontSize: '0.85rem', color: 'var(--color-body)', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>{typeof val === 'object' ? JSON.stringify(val) : String(val || '')}</span> },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search portfolio configs..." style={{ width: '100%', padding: '0.625rem 1rem 0.625rem 2.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: '0.875rem', outline: 'none' }} />
        </div>
      ...
      <ConfigTable columns={columns} data={configs} loading={isLoading} onEdit={openEdit} onDelete={handleDelete} onToggle={handleToggle} emptyTitle="No portfolio configs" emptyDescription="Set up portfolio display preferences." />
      ...
      <ConfigModal open={modalOpen} onClose={() => setModalOpen(false)} title={editingItem ? 'Edit Portfolio Config' : 'Add Portfolio Config'} loading={loading} footer={
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <input type="text" value={form.key || ''} onChange={(e) => setForm({ ...form, key: e.target.value })} placeholder="e.g. auto_publish" style={{ width: '100%', padding: '0.625rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: '0.875rem', outline: 'none' }} />
        </div>
        <div style={{ marginBottom: '1rem' }}>
